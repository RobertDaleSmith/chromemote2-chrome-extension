		...
		return $(this).each(function(){
			...
			if (!$this.hasClass('ip-enabled')) {
				...
				for (var i = 0; i <=3; i++) {
					...
					octets.push('<input type="text" class="ip_octet" id="' + id_prefix + '_octet_' + (i+1) + '" maxlength="3" placeholder="'+ placeHolderText +'" value="' + ip_value[i] + '" />');
				}
				var octet_html = octets.join('.');
				$this.after($('<div class="ip_container" style="display: inline' + ((false) ? '' : '-block') + ';"/>').html(octet_html));
